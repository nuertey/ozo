...
template <typename T>
inline native_cancel_handle pq_get_cancel(T& conn) {
    static_assert(Connection<T>, "T must be a Connection");
    return native_cancel_handle{PQgetCancel(get_native_handle(conn))};
}
...
inline bool pq_cancel(native_cancel_handle& h, std::string& err) {
...
inline auto dispatch_cancel(Handle h) {
    ...
    const auto pos = std::get<std::string>(res).find_last_not_of('\0');
    if (pos != std::string::npos) {
        std::get<std::string>(res).resize(pos);
    }
    return res;
}
